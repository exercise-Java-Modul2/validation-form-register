...
import com.codegym.service.UserServiceIplm;
...
public class UserController {
    UserService userService = new UserServiceIplm();
    ...
    public String save(@Valid @ModelAttribute("user") User user, BindingResult bindingResult, RedirectAttributes redirect) {
        ...
        if (bindingResult.hasFieldErrors()) {
            ...
            redirect.addFlashAttribute("success", "Created new user successfully");
            return "redirect:/";
        }
    }
    ...
    public String update(@Valid @ModelAttribute("user") User user, BindingResult bindingResult, RedirectAttributes redirect) {
        new User().validate(user, bindingResult);
        if (bindingResult.hasFieldErrors()) {
            return "update";
        } else {
            this.userService.update(user.getId(), user);
            redirect.addFlashAttribute("success", "Updated information successfully");
            return "redirect:/";
        }
    }
